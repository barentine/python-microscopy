...
class AndorBase(SDK3Camera, CameraMapMixin):
    ...
    PixelEncodingForGain = {
        '14-bit, 10 MHz': 'Mono16',
        '14-bit, 20 MHz': 'Mono16',
        '16-bit, 1 MHz': 'Mono16',
    }
    ...
    def Init(self):
        ...
        try:
            self.SetSimpleGainMode('16-bit, 1 MHz')
        except:
...
#        return self.contMode
    ...
    def Shutdown(self):
        logger.info('Shutting down Andor Clara camera')
...
class MultiviewClara(MultiviewCameraMixin, AndorClara):
    def __init__(self, camNum, multiview_info):
        AndorClara.__init__(self, camNum)
        # default to the whole chip
        default_roi = dict(xi=0, xf=1392, yi=0, yf=1040)
        MultiviewCameraMixin.__init__(self, multiview_info, default_roi, 
                                      AndorClara)
