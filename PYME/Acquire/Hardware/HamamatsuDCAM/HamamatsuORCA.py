...
class HamamatsuORCA(HamamatsuDCAM, CameraMapMixin):
    ...
    def GetGlobalExposureDelay(self):
        """If the camera is free-running in rolling shutter mode, this returns
        the delay from first row until all rows are being exposed in units of
        seconds.
    ...
    def _set_trigger_start_mode(self):
        ...
        try:
            ...
            # Someimes TRIGGER ACTIVE is not writable
            pass
        self.setCamPropValue('TRIGGER SOURCE', DCAMPROP_TRIGGERSOURCE_EXTERNAL)
        # set to software just for testing
        # self.setCamPropValue('TRIGGER SOURCE', DCAMPROP_TRIGGERSOURCE_SOFTWARE)
...
class Fusion(HamamatsuORCA):
    ...
    def SetGainMode(self, mode):
        """change readout speed / gain mode
        ...
        mode : int
            ...
            3: Fast
        """
        self.setCamPropValue('READOUT SPEED', mode)
